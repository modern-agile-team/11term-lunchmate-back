...
describe('Friend Request Reject (e2e)', () => {
  ...
  it('친구 신청 거절 성공', async () => {
    const { requester, receiver, friendRequest } = await createPendingRequest();

    const response = await request(app.getHttpServer())
      .patch(`/api/v1/friends/requests/${friendRequest.body.id}/reject`)
    ...
    expect(response.body.status).toBe(FriendStatus.REJECTED);

    const savedRelation = await dataSource.getRepository(Friend).findOne({
      where: { id: friendRequest.body.id },
    });

    expect(savedRelation?.status).toBe(FriendStatus.REJECTED);
  });

  it('요청 받은 사용자만 거절 가능', async () => {
  ...
  it('존재하지 않는 친구 요청 거절 시 404', async () => {
    ...
    const response = await request(app.getHttpServer())
      .patch('/api/v1/friends/requests/999999/reject')
      .set('Authorization', `Bearer ${receiver.body.accessToken}`);
